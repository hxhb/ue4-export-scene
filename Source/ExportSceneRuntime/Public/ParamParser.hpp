...
/*-----------------------------------------------------------------------------
	...
	fetched, or 0 if not.
-----------------------------------------------------------------------------*/
DEFINE_LOG_CATEGORY_STATIC(LogExportSceneParamParser, Log, All);
...
const TCHAR* GetFVECTORSpaceDelimited( const TCHAR* Stream, FVector& Value )
{
	...
	Value.X = FCString::Atof(Stream);
	//UE_LOG(LogExportSceneParamParser, Warning, TEXT("Value.X %f"), Value.X );
	Stream = FCString::Strchr(Stream,' ');
	if( !Stream )
	{
		return Stream;
	...
	Stream++;
	Value.Y = FCString::Atof(Stream);
	//UE_LOG(LogExportSceneParamParser, Warning, TEXT("Value.Y %f"), Value.Y );
	Stream = FCString::Strchr(Stream,' ');
	if( !Stream )
	{
		return Stream;
	...
	Stream++;
	Value.Z = FCString::Atof(Stream);
	//UE_LOG(LogExportSceneParamParser, Warning, TEXT("Value.Z %f"), Value.Z );
...
)
{
	...
	Rotation = FRotator::ZeroRotator;
	Rotation.Pitch = FCString::Atof(Stream) * ScaleFactor;
	//UE_LOG(LogExportSceneParamParser, Warning, TEXT("Rotation.Pitch %d"), Rotation.Pitch );
	Stream = FCString::Strchr(Stream,' ');
	if( !Stream )
	{
		return Stream;
	...
	Rotation.Yaw = FCString::Atof(++Stream) * ScaleFactor;
	//UE_LOG(LogExportSceneParamParser, Warning, TEXT("Rotation.Yaw %d"), Rotation.Yaw );
	Stream = FCString::Strchr(Stream,' ');
	if( !Stream )
	{
		return Stream;
	...
	Rotation.Roll = FCString::Atof(++Stream) * ScaleFactor;
	//UE_LOG(LogExportSceneParamParser, Warning, TEXT("Rotation.Roll %d"), Rotation.Roll );
